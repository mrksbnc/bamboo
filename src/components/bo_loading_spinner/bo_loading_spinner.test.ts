...
import { BoLoaderTextPosition, BoLoaderVariant, BoSize } from '@/shared'
...
describe('bo_loading_spinner.vue', () => {
	...
	test('the spinner animation should have the default classes', () => {
		...
		expect(globalWrapper.find('.bo-loading-spinner__animation').classes()).toContain('animate-spin')
		expect(globalWrapper.find('.bo-loading-spinner__animation').classes()).toContain('rounded-full')
		expect(globalWrapper.find('.bo-loading-spinner__animation').classes()).toContain('border-[2px]')
